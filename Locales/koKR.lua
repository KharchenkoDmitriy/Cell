...
L["raidStyleSwitchChatMsg"] = "전통적인 파티 프레임은 알려진 블리자드 오류(태인트)를 유발합니다. 이를 방지하기 위해 Cell이 자동으로 공격대 스타일 파티 프레임으로 전환했습니다. 변경 사항을 적용하려면 UI 재시작(리로드)이 필요합니다."
L["raidStyleSwitchPopupText"] = "전통적인 파티 프레임은 알려진 블리자드 오류를 유발합니다.\n\n이를 방지하기 위해 Cell이 자동으로 공격대 스타일 파티 프레임으로 전환했습니다.\n\n변경 사항을 적용하려면 UI 재시작(리로드)이 필요합니다.\n "
L["bossDebuffsTabNotice"] = "사용자 지정 공격대 디버프 목록은 더 이상 Retail(12.1+)에서 작동하지 않습니다\n\n블리자드가 파티 및 공격대 프레임에서 많은 전투 오라 주문 ID를 숨기기 때문에, 애드온이 더 이상 사용자의 활성화/비활성화 목록으로 안정적으로 필터링할 수 없습니다.\n\n변경 사항:\n• 기존 \"공격대 디버프\" 지표는 이제 \"Highlight Debuffs\"로 이름이 바뀌었습니다.\n• 아래에서 체크한 필터 클래스(블리자드가 제공하는 플래그, 예: isBossAura)에 해당하는 해로운 오라를 표시합니다.\n• 그 외의 모든 것은 일반 디버프 지표에 그대로 유지됩니다.\n\n이 탭의 주문 목록, 체크박스, 순서는 더 이상 Retail 프레임에 영향을 주지 않습니다. Classic과 MoP는 변경되지 않았습니다."
L["nonPlayerAurasTip"] = "기력 재충전 후유증(탈진, 포만감, 관용, 시간 왜곡)과 지옥 갑옷을 숨깁니다. 조우 디버프는 계속 표시됩니다."
L["debuffTooltipMouseoverTip"] = "여기서 아이콘 위에 마우스를 올리면 해당 위치에서 자신의 @mouseover 매크로/주문이 차단됩니다. 이 옵션을 완전히 피하려면 체크를 해제한 상태로 두세요."
L["keepInHealersTip"] = "활성화하면 이 버프가 힐러 지표에도 계속 표시됩니다.|비활성화하면 여기에서만 표시되고 힐러 지표에서는 제거됩니다."
L["glowDisabledApiWarning"] = "안타깝게도 API 변경으로 인해 현재 발광 효과를 구현할 수 없습니다. 해결 방법을 찾을 때까지 이 지표를 임시로 비활성화했습니다."
L["crowdControlsApiWarning"] = "Cell은 더 이상 사용자 지정 CC 목록을 사용할 수 없습니다. 블리자드가 어떤 디버프가 군중 제어인지 결정하며, 이 지표는 이를 자동으로 표시합니다."
L["bossDebuffsApiWarning"] = "Highlight Debuffs는 아래에서 체크된 필터 중 하나라도 일치하는 해로운 오라를 표시합니다. 체크된 필터에 해당하지 않는 것은 일반 디버프 지표에 그대로 유지됩니다. 사용자 지정 주문 목록은 더 이상 Retail(12.1+)에서 이를 제어할 수 없습니다."
L["highlightDebuffFiltersTip"] = "체크된 필터는 서로 결합되며 상호 배타적입니다: 일치 항목은 (위에서 아래로) 처음 체크된 필터 아래에만 표시되며, 두 번 표시되지 않습니다. 아무것도 체크하지 않으면 아무것도 표시되지 않습니다."
L["highlightDebuffFiltersTip_nonplayer"] = "당신이나 당신의 소환수가 시전하지 않은 디버프를 표시합니다."
L["highlightDebuffFiltersTip_priority"] = "블리자드가 공격대 프레임에 중요하다고 표시한 디버프를 표시합니다(isPriorityAura)."
L["highlightDebuffFiltersTip_cc"] = "군중 제어(기절, 공포, 속박 등)인 디버프를 표시합니다."
L["highlightDebuffFiltersTip_bossaura"] = "블리자드가 보스 오라(던전/공격대 보스)로 표시한 디버프를 표시합니다."
L["highlightDebuffFiltersTip_roleaura"] = "블리자드가 당신의 역할과 관련이 있다고 표시한 디버프를 표시합니다(예: 탱커 디버프)."
L["highlightDebuffFiltersTip_raid"] = "블리자드가 공격대와 관련이 있다고 표시한 디버프를 표시합니다."
L["highlightDebuffFiltersTip_raidcombat"] = "전투 중일 때 공격대 관련 디버프를 표시합니다."
L["highlightDebuffFiltersTip_dispellable"] = "지금 당신이 해제/정화할 수 있는 디버프를 표시합니다."
L["highlightDebuffFiltersTip_dispeltyped"] = "해제 가능 여부와 상관없이 해제 유형(마법/저주/질병/독/출혈)이 있는 모든 디버프를 표시합니다."
L["Fade at Full Health"] = "체력 가득 찰 때 흐리게"
L["healthFadeTip"] = "유닛의 체력이 아래 설정한 임계값을 넘으면 해당 프레임을 ('범위 밖 투명도' 값을 사용하여) 흐리게 만듭니다. 건강한 유닛은 뒤로 물러나고, 다친 유닛은 눈에 띄게 됩니다."
L["Health Fade Threshold"] = "체력 흐림 임계값"