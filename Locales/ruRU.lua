...
L["raidStyleSwitchChatMsg"] = "Традиционные фреймы группы вызывают известные ошибки Blizzard (порча/taint). Cell автоматически переключился на фреймы группы в стиле рейда, чтобы избежать этого. Для применения изменений требуется перезагрузка интерфейса."
L["raidStyleSwitchPopupText"] = "Традиционные фреймы группы вызывают известные ошибки Blizzard.\n\nCell автоматически переключился на фреймы группы в стиле рейда, чтобы избежать этого.\n\nДля применения изменений требуется перезагрузка интерфейса.\n "
L["bossDebuffsTabNotice"] = "Пользовательские списки рейдовых дебаффов больше не работают в Retail (12.1+)\n\nBlizzard скрывает многие ID заклинаний боевых аур на групповых и рейдовых фреймах, поэтому аддоны больше не могут надёжно фильтровать по вашему собственному списку включения/отключения.\n\nЧто изменилось:\n• Старый индикатор «Рейдовые дебаффы» теперь называется «Highlight Debuffs».\n• Он показывает вредные ауры, соответствующие отмеченным ниже классам фильтров (флаги, предоставляемые Blizzard, например isBossAura).\n• Всё остальное остаётся в обычном индикаторе дебаффов.\n\nСписок заклинаний, флажки и порядок на этой вкладке больше не влияют на фреймы Retail. Classic и MoP не изменились."
L["nonPlayerAurasTip"] = "Скрывает последствия эффектов ярости, таких как Истощение, Насыщение, Снисхождение и Искажение времени, а также Броню Скверны. Дебаффы столкновений остаются видимыми."
L["debuffTooltipMouseoverTip"] = "Наведение курсора на значок здесь заблокирует ваши собственные макросы/заклинания @mouseover в этом месте. Оставьте этот флажок снятым, чтобы полностью избежать этого."
L["keepInHealersTip"] = "Если включено, этот баф также остаётся видимым в индикаторе Целителей.|Если отключено, он отображается только здесь и удаляется из индикатора Целителей."
L["glowDisabledApiWarning"] = "К сожалению, свечение сейчас невозможно реализовать из-за изменений API. Я временно отключил этот индикатор, пока не будет найдено решение."
L["crowdControlsApiWarning"] = "Cell больше не может использовать собственный список контроля толпы. Blizzard решает, какие дебаффы являются контролем толпы, и этот индикатор показывает их автоматически."
L["bossDebuffsApiWarning"] = "Highlight Debuffs показывает вредные ауры, соответствующие любому из отмеченных ниже фильтров. Всё, что не попадает под отмеченный фильтр, остаётся в обычном индикаторе дебаффов. Пользовательские списки заклинаний больше не могут управлять этим в Retail (12.1+)."
L["highlightDebuffFiltersTip"] = "Отмеченные фильтры комбинируются и исключают друг друга: совпадение отображается только под первым подходящим фильтром (сверху вниз), никогда дважды. Ничего не отмечено = ничего не показывается."
L["highlightDebuffFiltersTip_nonplayer"] = "Показывает дебаффы, наложенные НЕ вами и не вашим питомцем."
L["highlightDebuffFiltersTip_priority"] = "Показывает дебаффы, которые Blizzard отмечает как важные для рейдовых фреймов (isPriorityAura)."
L["highlightDebuffFiltersTip_cc"] = "Показывает дебаффы контроля толпы (оглушение, страх, корни и т.д.)."
L["highlightDebuffFiltersTip_bossaura"] = "Показывает дебаффы, которые Blizzard отмечает как ауры боссов (подземелье/рейд)."
L["highlightDebuffFiltersTip_roleaura"] = "Показывает дебаффы, которые Blizzard отмечает как значимые для вашей роли (например, дебаффы танка)."
L["highlightDebuffFiltersTip_raid"] = "Показывает дебаффы, помеченные Blizzard как важные для рейда."
L["highlightDebuffFiltersTip_raidcombat"] = "Показывает важные для рейда дебаффы, пока вы в бою."
L["highlightDebuffFiltersTip_dispellable"] = "Показывает дебаффы, которые ВЫ можете снять/очистить прямо сейчас."
L["highlightDebuffFiltersTip_dispeltyped"] = "Показывает любой дебафф с типом снятия (Магия/Проклятие/Болезнь/Яд/Кровотечение), независимо от того, можете ли вы его снять."
L["Fade at Full Health"] = "Затухание при полном здоровье"
L["healthFadeTip"] = "Затемняет фрейм юнита (используя значение 'Вне зоны действия: прозрачность'), когда его здоровье поднимается выше указанного порога — здоровые юниты уходят на задний план, а раненые выделяются."
L["Health Fade Threshold"] = "Порог затухания по здоровью"