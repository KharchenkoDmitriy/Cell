...
L["raidStyleSwitchChatMsg"] = "传统队伍框体会导致已知的暴雪错误（taint）。Cell 已自动切换为团队样式的队伍框体以避免此问题。需要重新加载界面才能使更改生效。"
L["raidStyleSwitchPopupText"] = "传统队伍框体会导致已知的暴雪错误。\n\nCell 已自动切换为团队样式的队伍框体以避免此问题。\n\n需要重新加载界面才能使更改生效。\n "
L["bossDebuffsTabNotice"] = "自定义团队减益列表在正式服 (12.1+) 上不再起作用\n\n暴雪隐藏了小队和团队框体上许多战斗光环的法术ID，因此插件无法再依据你自己的启用/禁用列表进行可靠的过滤。\n\n变更内容：\n• 旧的“团队减益”指示器现已更名为“Highlight Debuffs”。\n• 它会显示符合下方勾选的过滤类别的有害光环（由暴雪提供的标记，例如 isBossAura）。\n• 其余的减益效果仍显示在普通的减益指示器中。\n\n此标签页中的法术列表、勾选框和排序对正式服框体已不再有任何效果。经典旧世和熊猫人之谜不受影响。"
L["nonPlayerAurasTip"] = "隐藏诸如力竭、饱腹、宽恕和时间扭曲之类的嗜血后遗症，以及邪甲术。团队遭遇减益效果仍然可见。"
L["debuffTooltipMouseoverTip"] = "将鼠标悬停在此处的图标上，会阻挡你自己在该位置的 @mouseover 宏/技能。不勾选此项可完全避免这个问题。"
L["keepInHealersTip"] = "启用后，该增益效果也会在治疗者指示器中保持可见。|禁用后，它只会在此处显示，并从治疗者指示器中移除。"
L["glowDisabledApiWarning"] = "很遗憾，由于API的变化，目前无法实现发光效果。我暂时禁用了此指示器，直到找到解决方案为止。"
L["crowdControlsApiWarning"] = "Cell 已无法再使用自定义的控制技能列表。暴雪决定哪些减益是控制类效果，此指示器会自动显示它们。"
L["bossDebuffsApiWarning"] = "Highlight Debuffs 会显示符合下方任一勾选过滤条件的有害光环。未被勾选过滤条件捕获的效果仍显示在普通减益指示器中。自定义法术列表在正式服 (12.1+) 上已无法再控制此项。"
L["highlightDebuffFiltersTip"] = "已勾选的过滤条件会互相组合并排除：一个匹配项只会显示在第一个匹配的已勾选过滤条件下（自上而下），绝不会重复显示。未勾选任何项 = 不显示任何内容。"
L["highlightDebuffFiltersTip_nonplayer"] = "显示并非由你或你的宠物施放的减益效果。"
L["highlightDebuffFiltersTip_priority"] = "显示暴雪标记为对团队框体重要的减益效果（isPriorityAura）。"
L["highlightDebuffFiltersTip_cc"] = "显示属于控制类效果的减益（眩晕、恐惧、定身等）。"
L["highlightDebuffFiltersTip_bossaura"] = "显示暴雪标记为首领光环的减益效果（地下城/团队副本首领）。"
L["highlightDebuffFiltersTip_roleaura"] = "显示暴雪标记为与你的职责相关的减益效果（例如坦克减益）。"
L["highlightDebuffFiltersTip_raid"] = "显示暴雪标记为团队相关的减益效果。"
L["highlightDebuffFiltersTip_raidcombat"] = "在你处于战斗状态时显示团队相关的减益效果。"
L["highlightDebuffFiltersTip_dispellable"] = "显示你当前可以驱散/净化的减益效果。"
L["highlightDebuffFiltersTip_dispeltyped"] = "显示任何具有驱散类型（法术/诅咒/疾病/中毒/流血）的减益效果，无论你是否能够移除它。"
L["Fade at Full Health"] = "满血时淡出"
L["healthFadeTip"] = "当单位生命值超过下方设定的阈值时，淡化该单位的框体（使用\"超出距离透明度\"的数值），使健康的单位淡入背景，受伤的单位则更加突出。"
L["Health Fade Threshold"] = "生命值淡出阈值"
