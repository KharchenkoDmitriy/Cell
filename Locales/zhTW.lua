...
L["raidStyleSwitchChatMsg"] = "傳統隊伍框架會導致已知的暴雪錯誤（taint）。Cell 已自動切換為團隊樣式的隊伍框架以避免此問題。需要重新載入介面才能使變更生效。"
L["raidStyleSwitchPopupText"] = "傳統隊伍框架會導致已知的暴雪錯誤。\n\nCell 已自動切換為團隊樣式的隊伍框架以避免此問題。\n\n需要重新載入介面才能使變更生效。\n "
L["bossDebuffsTabNotice"] = "自訂團隊減益列表在正式服 (12.1+) 上不再有效\n\n暴雪隱藏了隊伍與團隊框架上許多戰鬥光環的法術ID，因此外掛程式已無法再依據你自己的啟用/停用清單進行可靠的過濾。\n\n變更內容：\n• 舊的「團隊減益」指示器現在改名為「Highlight Debuffs」。\n• 它會顯示符合下方勾選過濾類別的有害光環（由暴雪提供的標記，例如 isBossAura）。\n• 其餘的減益效果仍維持在一般的減益指示器中。\n\n此分頁中的法術清單、勾選框與排序，對正式服框架已不再有任何作用。經典舊世與潘達利亞之謎不受影響。"
L["nonPlayerAurasTip"] = "隱藏諸如力竭、飽足、寬恕和時間扭曲之類的嗜血後遺症，以及邪甲術。團隊遭遇減益效果仍然可見。"
L["debuffTooltipMouseoverTip"] = "將滑鼠懸停在此處的圖示上，會阻擋你自己在該位置的 @mouseover 巨集/技能。不勾選此項可完全避免這個問題。"
L["keepInHealersTip"] = "啟用後，該增益效果也會在治療者指示器中保持可見。|停用後，它只會在此處顯示，並從治療者指示器中移除。"
L["glowDisabledApiWarning"] = "很遺憾，由於 API 的變化，目前無法實作發光效果。我暫時停用了這個指示器，直到找到解決方案為止。"
L["crowdControlsApiWarning"] = "Cell 已無法再使用自訂的控場技能清單。暴雪決定哪些減益是控場效果，此指示器會自動顯示它們。"
L["bossDebuffsApiWarning"] = "Highlight Debuffs 會顯示符合下方任一勾選過濾條件的有害光環。未被勾選過濾條件捕捉到的效果仍顯示在一般減益指示器中。自訂法術清單在正式服 (12.1+) 上已無法再控制此項目。"
L["highlightDebuffFiltersTip"] = "已勾選的過濾條件會互相組合並排除：一個符合項目只會顯示在第一個符合的已勾選過濾條件下（由上而下），絕不會重複顯示。未勾選任何項目 = 不顯示任何內容。"
L["highlightDebuffFiltersTip_nonplayer"] = "顯示並非由你或你的寵物施放的減益效果。"
L["highlightDebuffFiltersTip_priority"] = "顯示暴雪標記為對團隊框架重要的減益效果（isPriorityAura）。"
L["highlightDebuffFiltersTip_cc"] = "顯示屬於控場效果的減益（暈眩、恐懼、定身等）。"
L["highlightDebuffFiltersTip_bossaura"] = "顯示暴雪標記為首領光環的減益效果（地下城/團隊副本首領）。"
L["highlightDebuffFiltersTip_roleaura"] = "顯示暴雪標記為與你的職責相關的減益效果（例如坦克減益）。"
L["highlightDebuffFiltersTip_raid"] = "顯示暴雪標記為團隊相關的減益效果。"
L["highlightDebuffFiltersTip_raidcombat"] = "在你處於戰鬥狀態時顯示團隊相關的減益效果。"
L["highlightDebuffFiltersTip_dispellable"] = "顯示你目前可以驅散/淨化的減益效果。"
L["highlightDebuffFiltersTip_dispeltyped"] = "顯示任何具有驅散類型（法術/詛咒/疾病/中毒/流血）的減益效果，無論你是否能夠移除它。"
L["Fade at Full Health"] = "滿血時淡出"
L["healthFadeTip"] = "當單位生命值超過下方設定的閾值時，淡化該單位的框架（使用\"超出範圍透明度\"的數值），使健康的單位淡入背景，受傷的單位則更加突出。"
L["Health Fade Threshold"] = "生命值淡出閾值"