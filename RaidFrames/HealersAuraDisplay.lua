...
--[[
    Always-on Healers AuraContainer (12.1+): enabled OOC and in combat.
    Legacy Healers icons stay hidden once the container exists.
]]
...
local function ResolveAnimationStyle(cfg)
    ...
    -- Legacy checkbox: false = none; otherwise prefer circular swipe for Healers.
...
local function AttachInvisibleCooldown(button)
    -- Consume Blizzard's duration-cooldown slot so the default swipe cannot appear.
...
local function InitAuraButton(button)
    ...
    elseif style == "vertical" and type(button.SetDurationBar) == "function" then
        ...
        -- Elapsed time fills top→bottom (dark = spent, bright = remaining) like Cell preview.
    ...
    if not cfg or cfg.showStack ~= false then
        ...
        -- Empty opts: Blizzard only paints applications > 1 (same as Cell stacks).
        pcall(button.SetApplicationCount, button, stack, {})
    end

    -- false = never; true = always (simple checkbox in Healers settings).
...
if SUPPORTED then
    ...
    boot:SetScript("OnEvent", function(_, event)
        ...
        -- Always-on: re-sync / finish builds; never fall back to legacy paint.
        F.IterateAllUnitButtons(function(b)
            SyncButton(b, true)
        end, true)
    end)
