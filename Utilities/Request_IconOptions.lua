...
local function UpdateIconPreview()
    ...
    F.BD(previewButton).widgets.srIcon:SetAnimationType(setting[1])
    P.Size(F.BD(previewButton).widgets.srIcon, setting[2], setting[2])
    P.ClearPoints(F.BD(previewButton).widgets.srIcon)
    P.Point(F.BD(previewButton).widgets.srIcon, setting[3], F.BD(previewButton).widgets.srGlowFrame, setting[4], setting[5], setting[6])
    F.BD(previewButton).widgets.srIcon:Display(icon, iconColorTable)
end
